...
function Card({
  ...
  return (
    <section
      className={[
        "rounded-2xl border border-[#F6F6F6] bg-[#F6F6F6]",
        "p-4 md:p-5",
        className,
      ].join(" ")}
    >
...
function Chip({
  ...
  return (
    <span className="profileSkills inline-flex items-center gap-2 rounded-md border border-white bg-white px-3 py-1">
      {children}
      {onRemove ? (
        <button type="button" aria-label="remove" className="-mt-0.5" onClick={onRemove}>
          ×
        </button>
      ) : null}
    </span>
...
export default function ProfilePage() {
  /* Personal info is static in this UI */
  ...
  const [selectedDays, setSelectedDays] = useState<Day[]>(["Monday", "Friday", "Sunday"]);
  ...
  const [availItems, setAvailItems] = useState<
    { id: string; days: Day[]; slots: string[] }[]
  ...
  const removeLoc = (l: string) => setLocs((prev) => prev.filter((x) => x !== l));
  ...
  const removePref = (p: string) => setPrefs((prev) => prev.filter((x) => x !== p));
              ...
              <>
                {workItems.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {workItems.map((w) => (
                      <li key={w.id} className="rounded-md bg-white p-3 border border-white">
                        ...
                        {!w.collapsed && (
                          <div className="mt-2 text-sm opacity-80">
                            <div>{w.company}</div>
                            <div>
                              {w.startMonth} {w.startYear} – {w.endMonth} {w.endYear}
                            </div>
                            {w.description ? <div className="mt-1">{w.description}</div> : null}
                          </div>
                        )}
                      </li>
                    ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setWorkForm((f) => ({ ...f, jobTitle: e.target.value }))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <select
                      ...
                      onChange={(e) =>
                        setWorkForm((f) => ({ ...f, startMonth: e.target.value }))
                      }
                      className="w-full h-10 rounded-md border border-white bg-white px-3 outline-none"
                    >
                      {months.map((m) => (
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setWorkForm((f) => ({ ...f, startYear: e.target.value }))
                      }
                      className="w-full h-10 rounded-md border border-white bg-white px-3 outline-none"
                    >
                      {years.map((y) => (
                ...
                <div>
                  <label className="block mb-1 profileSkills">Description</label>
                  <textarea
                ...
                <div className="flex gap-3">
                  <GhostButton onClick={() => setEditingWork(false)}>Cancel</GhostButton>
          ...
          >
            {!editingSkills ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <GhostButton onClick={() => setEditingSkills(false)}>Cancel</GhostButton>
                  <RedButton onClick={() => setEditingSkills(false)}>Add</RedButton>
                </div>
              </div>
            )}
          </Card>
          ...
          >
            {!editingAvail ? (
              <div className="space-y-3">
                {availItems.map((a) => (
                  <div key={a.id} className="flex flex-wrap gap-2 items-center">
                    ...
                    <button className="text-sm underline ml-2" onClick={() => removeAvailability(a.id)}>
                      Remove
                    </button>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                    ...
                    return (
                      <button
                        ...
                        className={[
                          ...
                          checked ? "border-[#CC0000] bg-white" : "border-white bg-white/70",
                        ].join(" ")}
                      >
                        <span
                          className={[
                            "inline-flex items-center justify-center w-5 h-5 rounded-[4px] border",
                            checked ? "border-[#CC0000] bg-[#CC0000]" : "border-[#667282] bg-[#EDEDED]",
                          ].join(" ")}
                        >
                          {checked ? (
                            <svg
                ...
                <div className="flex gap-3">
                  <GhostButton onClick={() => setEditingAvail(false)}>Cancel</GhostButton>
            ...
            {!editingLoc ? (
              ...
              <div className="space-y-3">
                <input
                  ...
                  onKeyDown={(e) => (e.key === "Enter" ? (e.preventDefault(), addLoc()) : null)}
                  placeholder="Type Here"
                  className="w-full h-10 rounded-md border border-white bg-white px-3 outline-none"
                />
                <div className="flex flex-wrap gap-2">
                ...
                <div className="flex gap-3">
                  <GhostButton onClick={() => setEditingLoc(false)}>Cancel</GhostButton>
                  <RedButton onClick={() => setEditingLoc(false)}>Add</RedButton>
                </div>
              </div>
            )}
          </Card>
          ...
          >
            {!editingJobs ? (
              <div className="flex flex-wrap gap-2">
                ...
                {prefs.length === 0 && (
                  <div className="text-sm opacity-70">No job preference found</div>
                )}
              </div>
            ) : (
              <div className="space-y-3">
                <input
                  ...
                  onKeyDown={(e) => (e.key === "Enter" ? (e.preventDefault(), addPref()) : null)}
                  placeholder="Type Here"
                  className="w-full h-10 rounded-md border border-white bg-white px-3 outline-none"
                />
                <div className="flex flex-wrap gap-2">
                ...
                <div className="flex gap-3">
                  <GhostButton onClick={() => setEditingJobs(false)}>Cancel</GhostButton>
                  <RedButton onClick={() => setEditingJobs(false)}>Add</RedButton>
                </div>
              </div>
            )}
          </Card>
          ...
          <Card>
            ...
            <div className="mt-2">
              ...
              <div className="h-2 rounded-full bg-[#ECECEC] overflow-hidden">
                <div className="h-2 bg-[#CC0000] rounded-full" style={{ width: "90%" }} />
              </div>
            </div>
