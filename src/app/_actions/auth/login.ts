...
export async function login(formData: FormData) {
  ...
  if (user) {
    ...
    const { data: userMetadata, error: metadataError } =
      await AuthService.getUserMetadata(user.id);
    ...
    if (userMetadata) {
      ...
      if (userMetadata.role === "kindtao") {
        ...
        const onboardingProgress =
          await OnboardingService.checkOnboardingProgress(user.id);
        ...
        if (onboardingProgress.isComplete) {
          ...
          console.log(
            "Redirecting to next onboarding stage:",
            onboardingProgress.nextStage
          );
          revalidatePath("/", "layout");
          redirect(onboardingProgress.nextStage || "/onboarding/personal-info");
        }
      } else if (userMetadata.role === "kindbossing") {
        console.log("User is kindbossing, checking family profile progress...");

        // Check family profile progress
        const familyProgress =
          await FamilyProfileService.checkFamilyOnboardingProgress(user.id);
        console.log("Family profile progress:", familyProgress);

        if (familyProgress.isComplete) {
          // Family profile complete, redirect to dashboard
          console.log("Family profile complete, redirecting to dashboard...");
          revalidatePath("/", "layout");
          redirect("/kindbossing-dashboard");
        } else {
          // Redirect to next incomplete family profile stage
          console.log(
            "Redirecting to next family profile stage:",
            familyProgress.nextStage
          );
          revalidatePath("/", "layout");
          redirect(
            familyProgress.nextStage || "/family-profile/household-info"
          );
        }
